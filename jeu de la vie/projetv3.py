...
f = open('start_cell.txt', 'r')

#   calculate number of line in document, the number of cells is number_of_cell*number_of_cell
number_of_cell = 0
for line in f:
    number_of_cell += 1
...
# create cells[number_of_cell][number_of_cell] to display and cells older
cells       = [[0 for j in range(number_of_cell)] for i in range(number_of_cell)]
cells_older = [[0 for j in range(number_of_cell)] for i in range(number_of_cell)]

#   we read all cells in document and we initialize cells tab
count = 0
...
def show_cell():
    ...
    for x in range(number_of_cell):
        ...
        for y in range(number_of_cell):
            ...
            if (cells[x][y] == 1) :
                p.draw.circle(root , (cells_older[x][y], 0, cells_older[x][y]), (x*size_of_cell, y*size_of_cell), 6)
            else :
                p.draw.circle(root , WHITE, (x*size_of_cell, y*size_of_cell), 1) 


# void loop of this program
while 1:    
